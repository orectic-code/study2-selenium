...
# Chromeを起動する関数
def setup_driver(driver_path,headless_flg):
    # Chromeドライバーの読み込み
    ...
    return Chrome(ChromeDriverManager().install(),option=options)
...
def main():
    ...
    fail = 0
    while True:
        # 検索結果の一番上の会社名を取得(まれに1行目が広告の場合、おかしな動作をするためcassetteRecruit_headingで広告を除外している)
        name_list = driver.find_elements_by_css_selector("cassetteRecruit__heading .cassetRecruit_name")
        copy_list = driver.find_elements_by_css_selector("cassetteRecruit__heading .cassetRecruit_copy")
